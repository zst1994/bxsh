...
class DioManager {
  ...
  Future<T> get<T>(String url, Map<String, dynamic> params,
    ...
    return _requestHttp<T>(showContext, baseUrl + url, onSuccess, loadingStr,
        onError, 'get', params);
  }
  ...
  Future<T> _requestHttp<T>(BuildContext _buildContext, String url,
      SuccessCallback<T> onSuccess, String loadingStr,
    ...
    try {
      ...
      debugPrint("basewrap:" + baseWrap.toString());
      if (!baseWrap.isOk()) {
        if (_buildContext != null) {
          dismissDialog(_buildContext);
        }
  ...
  void _getReqConst() async {
    baseUrl = "https://wxmini.baixingliangfan.cn";
    // Logger.log(baseUrl);
    // Logger.log(reqCons.toString());
  }
